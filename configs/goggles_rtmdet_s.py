...
# Single RTX 2050, 4 GiB. Source paths are relative to the project root.
model = dict(
...
metainfo = dict(classes=('goggles',), palette=[(0, 255, 0)])
train_pipeline = [
...
train_dataloader = dict(
    ...
    dataset=dict(data_root='', ann_file='data/goggles/annotations/train.json',
                 data_prefix=dict(img='data/goggles/images/train/'),
...
val_dataloader = dict(
    ...
    dataset=dict(data_root='', ann_file='data/goggles/annotations/valid.json',
                 data_prefix=dict(img='data/goggles/images/valid/'), metainfo=metainfo))
test_dataloader = dict(
    batch_size=1, num_workers=0, persistent_workers=False,
    dataset=dict(data_root='', ann_file='data/goggles/annotations/test.json',
                 data_prefix=dict(img='data/goggles/images/test/'), metainfo=metainfo))
val_evaluator = dict(ann_file='data/goggles/annotations/valid.json', metric='bbox', proposal_nums=(100, 300, 1000))
test_evaluator = dict(ann_file='data/goggles/annotations/test.json', metric='bbox', proposal_nums=(100, 300, 1000))
